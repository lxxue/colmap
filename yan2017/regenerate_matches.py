...
def ConstructPathNetwork(num_images, matches_list, img_included, unique_tracks, visible_tracks, minimal_views, score_thres):
    ...
    for i in range(num_images):
        if img_included[i]:
            ...
            # TODO: if we use continue here, then non-iconic images will also be used for connection
            # continue
        else:
        ...
        for j, _ in matches_list[i]:
            ...
            if len(unique_ij) > minimal_views:
                ...
                if score > score_thres:
                    print("{}({} {} {:.2f})".format(j, len(unique_ij), max(len(unique_i), len(unique_j)), score))
